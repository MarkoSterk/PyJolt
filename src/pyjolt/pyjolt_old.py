...
class PyJolt:
    ...
    def _initialize_jinja2(self):
        ...
        self.render_engine = Environment(
            ...
            if self._configs.get("TEMPLATES_STRICT", True)
            else Undefined,
        )
    ...
    def _add_exception_handler(self, handler: Callable, exception: Exception):
        ...
        if handler_name in self._registered_exception_handlers:
            raise DuplicateExceptionHandler(
                f"Duplicate exception handler name {handler_name}"
            )
    ...
    async def _base_app(self, req: Request, res: Response, send):
        ...
        """
        try:
            res = await run_sync_or_async(
                req.route_handler, req, res, **req.route_parameters
            )
        except (
            StaticAssetNotFound,
            SchemaValidationError,
            ...
            res = res.json(
                {"status": exc.status, "message": exc.message, "data": exc.data}
            ).status(exc.status_code)
            # pylint: disable-next=W0718
        except Exception as exc:
            if exc.__class__.__name__ in self._registered_exception_handlers:
                res = await self._registered_exception_handlers[exc.__class__.__name__](
                    req, res, exc
                )
            else:
                raise
    ...
    def build(self) -> None:
        ...
        if self.get_conf("OPEN_API"):
            ...
            self._add_route_function(
                "GET", self.get_conf("OPEN_API_JSON_URL"), open_api_json_spec
            )
            self._add_route_function(
                "GET", self.get_conf("OPEN_API_SWAGGER_URL"), open_api_swagger
            )
        app = self._app
    ...
    def get_extension(self, ext_name: str | object):
        ...
        if not isinstance(ext_name, str):
            ext_name = (
                ext_name.__name__ if ext_name.__name__ else ext_name.__class__.__name__
            )
